...
class Tag2Text(nn.Module):
    ...
    def text_generate(self,
        ...
        image_embeds = self.visual_encoder(region)
        image_atts = torch.ones(image_embeds.size()[:-1],
                                dtype=torch.long).to(region.device)
        ...
        if tag_input == None:
            ...
            targets = torch.where(
                torch.sigmoid(logits) > self.class_threshold.to(region.device),
                torch.tensor(1.0).to(region.device),
                torch.zeros(self.num_class).to(region.device))
        ...
        # prompt trick for better captioning, followed BLIP
        # if insert_input == None:
        #     prompt = [self.prompt] * region.size(0)
        # else:
        #     prompt = ['a photo of the driving footage off'] * region.size(0)
        if insert_input == None:
            prompt = [self.prompt] * region.size(0)
        else:
            prompt = ['a cityscapes photo of'] * region.size(0)
        input_ids = self.tokenizer(prompt, return_tensors="pt").input_ids.to(
            region.device)
        input_ids[:, 0] = self.tokenizer.bos_token_id
        input_ids = input_ids[:, :-1]
